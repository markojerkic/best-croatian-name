...
import { getNamesFromIds, getRandomNames, updateRating } from "../../backend/names-functions";
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<NameSelection | {message: string}>
) {
  if (req.method !== 'POST' ) {
  ...
  const votedAgainsName = votedNames.filter((name) => name.id === vote.voteAgainst)[0];

  const newForRating = newScore(votedForName.rating!, votedAgainsName.rating!, 1);
  const newAgainsRating = newScore(votedAgainsName.rating!, votedForName.rating!, 0);
