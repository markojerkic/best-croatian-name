...
const castVote = async (voteFor: number, voteAgainst: number, 
    setNames: Dispatch<SetStateAction<NameSelection>>, setLoading: Dispatch<SetStateAction<boolean>>) => {
  setLoading(true);
  setNames(await axios.post<NameSelection>('/api/vote-names', {voteFor, voteAgainst}).then((res) => {
    setLoading(false);
    return res.data
  }));
}
    ...
    <div className="w-full h-screen flex flex-col justify-center space-y-6">
      ...
      <div className="md:space-x-4 flex flex-col lg:flex-row justify-center items-center">
        <NameChoice name={firstName.name} gender={firstName.gender}
          loading={loading}
          click={() => castVote(firstName.id, secondName.id, setNames, setLoading)} 
        />
        <div className="my-4">ili</div>
        <NameChoice name={secondName.name} gender={secondName.gender}
          loading={loading}
          click={() => castVote(secondName.id, firstName.id, setNames, setLoading)} 
        />
      </div>
    </div>
  )
